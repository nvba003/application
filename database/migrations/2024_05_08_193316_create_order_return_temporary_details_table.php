...
class CreateOrderReturnTemporaryDetailsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('order_return_temporary_details', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('order_return_temporary_id');
            $table->foreign('order_return_temporary_id')->references('id')->on('order_return_temporaries')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('order_return_temporary_details');
    }
}
