...
<div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white shadow-md rounded my-1 overflow-x-auto">
        <table class="min-w-full leading-normal" id="promotionTable">
            ...
            <tbody>
                @include('accounting.partials.order_return_temporary_table', ['orders' => $orders])
            </tbody>
        </table>
    </div>
...
$(document).ready(function() {
    ...
    fetchData('{{ route('orderReturnTemporary') }}');

    $('#searchForm').on('submit', function(e) {
        e.preventDefault();
        perPage = $('#perPage').val();
        currentSearchParams = updateSearchParams('per_page', perPage, $(this).serialize());
        fetchData('{{ route('orderReturnTemporary') }}?' + currentSearchParams);
    });
